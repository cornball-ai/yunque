...
lna <- as.array(layer_norm(x, weight = lw, bias = lb, eps = 1e-6))
ref <- t(apply(x_r, 1, function(r) {
    mu <- mean(r)
    v <- mean((r - mu)^2)
    (r - mu) / sqrt(v + 1e-6)
...
rn <- as.array(rms_norm(x, w, eps = 1e-6))
...
y <- as.array(linear(anvl::nv_array(x3_r, dtype = "f32"),
...
sk <- as.array(snake(x, a, bta, eps = 1e-9))
expect_true(max(abs(sk - (x_r + sin(x_r * amat)^2 / (bmat + 1e-9)))) < 1e-5)
# beta = alpha default
sk1 <- as.array(snake(x, a))
